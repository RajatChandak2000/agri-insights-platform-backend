...
export class ReceiptsService{
    ...
    async updateInput(email: string, updateDto: ReceiptsInputDto) {
        ...
        try {
            ...
            return await this.calculateReceiptsOutput(updatedDocument,productionDetailsInputs);
    ...
    async calculateReceiptsOutput(
        ...
        if ('userId' in updatedDocument) {
          this.logger.log(`Calculating recipts input details output for user: ${updatedDocument.userId}`);  
        ...
        const { expectedPercentMaleWithSexedSemen, expectedPercentMaleWithConventional, cullingRate, cowDeathLossRate, heiferRaisingDeathLossRate, bullCalfDeath, numberOfHeifersRaised } = productionDetailsInputs.heiferProduction;
        const { beefCrossPercent, beefCrossDeathRate } = productionDetailsInputs.beefCrossDetails;
        const expectedAnnualMilkSales = fetchedproductionDetailsOutputs.expectedAnnualMilkSales;
        ...
        if ('userId' in updatedDocument) {
          try {
            ...
            this.logger.log(`Successfully calculated and updated receipts output for user: ${updatedDocument.userId}`);
            return result;
          } catch (error) {
