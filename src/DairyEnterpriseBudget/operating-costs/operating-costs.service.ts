...
export class OperatingCostsService {
  ...
  constructor(
    ...
    @InjectModel(OperatingCostsInput.name)
    private operatingCostsInputModel: Model<OperatingCostsInput>,
    @InjectModel(OperatingCostsOutput.name)
    private operatingCostsOutputModel: Model<OperatingCostsOutput>,
    ...
    private reciptsOutputModel: Model<ReceiptsOutput>,
    @InjectModel(LabourCostInput.name)
    private labourinputmodel: Model<LabourCostInput>,
    @InjectModel(LabourCostOutput.name)
    private labouroutputmodel: Model<LabourCostOutput>,
  ...
  async updateInput(email: string, updateDto: OperatingCostsInputDto) {
    ...
    try {
      const updatedDocument =
        await this.operatingCostsInputModel.findOneAndUpdate(
          { userId },
          { $set: updateData },
      ...
      const reciptsInputs = await this.receiptsInputModel
        .findOne({ userId })
        .exec();

      const labourinputs = await this.labourinputmodel
        .findOne({ userId })
        .exec();

      return await this.calculateOperatingCostsOutput(
        updatedDocument,
  ...
  async calculateOperatingCostsOutput(
    ...
    if ('userId' in updatedDocument) {
      ...
      fetchedProductionDetailsOutputs = await this.productionDetailsOutputModel
        .findOne({ userId })
        .exec();
      fetchedReceiptsOutputs = await this.reciptsOutputModel
        .findOne({ userId })
        .exec();
      fetchedLabourOutputs = await this.labouroutputmodel
        .findOne({ userId })
        .exec();

      if (!fetchedProductionDetailsOutputs || !fetchedReceiptsOutputs) {
        this.logger.error(
          `No production details or receipts found for user: ${userId}`,
        );
        throw new Error(
          `No production details or receipts found for user: ${userId}`,
        );
      ...
      fetchedReceiptsOutputs =
        await this.ReceiptsService.calculateReceiptsOutput(
          receiptsInput,
          productionDetailsInputs,
        );
      if (labourinputs !== null) {
        fetchedLabourOutputs =
          await this.LabourCostService.calculateLabourCostOutput(labourinputs);
      }
    }
    ...
    console.log(
      'Here is fetchedReceiptsOutputs in number: ',
      fetchedReceiptsOutputs,
    );
    ...
    const totalAnnualMilkProduction = Number(
      fetchedProductionDetailsOutputs.totalAnnualMilkProduction,
    );
    const totalNumberOfCows = Number(
      productionDetailsInputs.milkProduction.totalNumberOfCows,
    );
    const numberOfLactationsPerYear = Number(
      fetchedProductionDetailsOutputs.numberOfLactationsPerYear,
    );
    const beefCrossBullsProduced = Number(
      fetchedReceiptsOutputs.beefCrossBullsProduced,
    );
    const beefCrossHeifersProduced = Number(
      fetchedReceiptsOutputs.beefCrossHeifersProduced,
    );
    ...
    const inseminationSexedCosts =
      ...
      ((1 / 0.47) *
        (2 / 3) *
        (numberOfLactationsPerYear -
          (Math.round(beefCrossBullsProduced) +
            Math.round(beefCrossHeifersProduced))));
    const inseminationConventionalCosts =
      inseminationConventionalFees *
      ((1 / 0.57) *
        (1 / 3) *
        (numberOfLactationsPerYear -
          (Math.round(beefCrossBullsProduced) +
            Math.round(beefCrossHeifersProduced))));
    const inseminationConventionalBeefCosts =
      inseminationConventionalBeefFees *
      ((1 / 0.57) *
        (Math.round(beefCrossBullsProduced) +
          Math.round(beefCrossHeifersProduced)));
    ...
    if (useDetailedLaborCost === false || fetchedLabourOutputs === null) {
      ...
      dairyPayroll =
        fetchedLabourOutputs.ownerLaborCost +
    ...
    const totalReceipts =
      fetchedReceiptsOutputs.milkSales +
    ...
    const additionalManagementCosts =
      (additionalManagementCostsPercentage / 100) * totalReceipts;
    ...
    const operatingCostsOutput = {
      ...
      additionalManagementCosts: parseFloat(
        additionalManagementCosts.toFixed(2),
      ),
      ...
      inseminationConventionalCosts: parseFloat(
        inseminationConventionalCosts.toFixed(2),
      ),
      inseminationConventionalBeefCosts: parseFloat(
        inseminationConventionalBeefCosts.toFixed(2),
      ),
    ...
    if ('userId' in updatedDocument) {
      try {
        ...
        this.logger.log(
          `Successfully calculated and updated operating costs output for user: ${updatedDocument.userId}`,
        );
        return result;
      } catch (error) {
        this.logger.error(
          `Failed to calculate operating costs output: ${error.message}`,
        );
        throw new Error(
          `Failed to calculate operating costs output: ${error.message}`,
        );
  ...
  async getOperatingCostOutput(
    email: string,
  ): Promise<OperatingCostsOutput | null> {
    ...
    const outputDocument = await this.operatingCostsOutputModel
      .findOne({ userId })
      .exec();
  ...
  async getOperatingCostInput(
    email: string,
  ): Promise<OperatingCostsInput | null> {
    //first find the user_id using the email, then find the document using the id
    const user = await this.userModel.findOne({ email }).exec();
    if (!user) {
      throw new HttpException(
    ...
    const inputDocument = this.operatingCostsInputModel
      .findOne({ userId })
      .exec();
