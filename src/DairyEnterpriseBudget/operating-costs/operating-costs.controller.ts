...
export class OperatingCostsController {
  ...
  async geOperatingCostDetailsOutput(
  ...
  async calculateOutputs(
    @Body()
    body: {
      inputs: OperatingCostsInputDto;
      productionDetails: ProductionDetailsInputDto;
      reciptsInputs: ReceiptsInputDto;
      labourInputs: LabourCostInputDto;
    },
  ) {
    ...
    console.log(
      'Calculating Operating Cost outputs for non-authenticated user',
    );
    return this.operatingCostsService.calculateOperatingCostsOutput(
      inputs,
      productionDetails,
      reciptsInputs,
      labourInputs,
