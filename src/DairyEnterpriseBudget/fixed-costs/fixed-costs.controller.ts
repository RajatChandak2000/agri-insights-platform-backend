...
export class FixedCostsController{
    ...
    async getFixedCostsOutput(@Param('email') email:string): Promise<FixedCostsOutput | null>{
        console.log('Api got called');
        return this.fixedCostsSerive.getFixedCostsOutput(email);
    }

    @Get('inputDetails/:email')
    async getFixedCostsInput(@Param('email') email:string): Promise<FixedCostsInput | null>{
        console.log('Api got called');
