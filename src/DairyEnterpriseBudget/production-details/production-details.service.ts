...
export class ProductionDetailsService {
  ...
  async updateInput(email: string, updateDto: ProductionDetailsInputDto) {
    ...
    if (updateDto.milkProduction) {
      ...
      this.logger.log(
        `Milk Production Data: ${JSON.stringify(updateDto.milkProduction)}`,
      );
    }
    ...
    if (updateDto.heiferProduction) {
      ...
      this.logger.log(
        `Heifer Production Data: ${JSON.stringify(updateDto.heiferProduction)}`,
      );
    }
    ...
    if (updateDto.beefCrossDetails) {
      ...
      this.logger.log(
        `Beef Cross Details Data: ${JSON.stringify(updateDto.beefCrossDetails)}`,
      );
    }

    try {
      const updatedDocument =
        await this.productionDetailsInputModel.findOneAndUpdate(
          { userId },
          { $set: updateData },
  ...
  async calculateProductionDetailsOutput(
    updatedDocument: ProductionDetailsInput | ProductionDetailsInputDto,
  ) {
    if ('userId' in updatedDocument) {
      this.logger.log(
        `Calculating production details output for user: ${updatedDocument.userId}`,
      );
    }

    // Inputs and temp variables required to calculate the outputs
    const expectedMilkProduction =
      updatedDocument.milkProduction.expectedMilkProduction;
    ...
    const rollingHerdAverage = (
      expectedMilkProduction /
      (calvingInterval / 12)
    ).toFixed(2);
    const totalAnnualMilkProduction = (
      (numberOfLactationsPerYear * expectedMilkProduction) /
      100
    ).toFixed(2);
    const expectedAnnualMilkSales = (
      (parseFloat(rollingHerdAverage) / 100) *
      expectedMilkPrice *
      totalNumberOfCows
    ).toFixed(2);
    ...
    if ('userId' in updatedDocument) {
      try {
        ...
        this.logger.log(
          `Successfully calculated and updated production details output for user: ${updatedDocument.userId}`,
        );
        return result;
      } catch (error) {
        this.logger.error(
          `Failed to calculate production details output: ${error.message}`,
        );
        throw new Error(
          `Failed to calculate production details output: ${error.message}`,
        );
  ...
  async getProductionDetailsOutput(
    ...
    const outputDocument = await this.productionDetailsOutputModel
      .findOne({ userId })
      .exec();
  ...
  async getProductionDetailsInput(
    ...
    const inputDocument = this.productionDetailsInputModel
      .findOne({ userId })
      .exec();
