...
export class DairyPricesController {
  ...
  @Get('year/:reportYear')
  async getDairyPricesByYear(
    @Param('reportYear') reportYear: number,
  ): Promise<DairyPrice[]> {
    const dairyPrices =
      await this.dairyPricesService.getDairyPricesByYear(reportYear);
    if (dairyPrices.length === 0) {
      throw new NotFoundException(
        `No dairy prices found for year ${reportYear}`,
      );
    }
